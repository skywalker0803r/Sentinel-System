...
class SmartMoneyConceptsAnalyzer:
    ...
    def detect_market_structure(self, df: pd.DataFrame, lookback: int = 50) -> Dict:
        ...
        # 計算 swing highs 和 swing lows
        df['swing_high'] = df['high'].rolling(window=lookback//2, center=True).max() == df['high']
        df['swing_low'] = df['low'].rolling(window=lookback//2, center=True).min() == df['low']
            ...
            if is_bullish_ob:
                ...
                if (ob_high - ob_low) > current_atr * 0.5:
                    order_blocks.append({
                        ...
                        'active': self._is_order_block_active(df, ob_low, ob_high, i),
            ...
            for j in range(i + 1, len(swing_highs)):
                ...
                if price_diff <= threshold * swing_highs[i]['price']:
                    equal_highs.append({
                        ...
                        'count': 2,
                        'type': 'EQUAL_HIGH',
                        'description': f'等高點 ${swing_highs[i]["price"]:.6f}'
                    })
            ...
            for j in range(i + 1, len(swing_lows)):
                ...
                if price_diff <= threshold * swing_lows[i]['price']:
                    equal_lows.append({
                        ...
                        'count': 2,
                        'type': 'EQUAL_LOW',
                        'description': f'等低點 ${swing_lows[i]["price"]:.6f}'
                    })
        ...
        for i in range(20, len(df) - 5):
            ...
            if len(prev_highs) > 0:
                ...
                if current_high > max_prev_high:
                    ...
                    if len(next_candles) > 0 and next_candles['close'].min() < max_prev_high:
                        sweeps.append({
                            'type': 'SELL_SIDE_LIQUIDITY',
                            'price': current_high,
                            'swept_level': max_prev_high,
                            'time': df.get('time', pd.Series([i] * len(df))).iloc[i],
                            'description': f'掃蕩賣方流動性 ${max_prev_high:.6f}'
                        })
            ...
            if len(prev_lows) > 0:
                ...
                if current_low < min_prev_low:
                    # 檢查後續是否快速反彈 (假跌破)
                    next_candles = df.iloc[i+1:i+6]
                    if len(next_candles) > 0 and next_candles['close'].max() > min_prev_low:
                        sweeps.append({
                            'type': 'BUY_SIDE_LIQUIDITY',
                            'price': current_low,
                            'swept_level': min_prev_low,
                            'time': df.get('time', pd.Series([i] * len(df))).iloc[i],
                            'description': f'掃蕩買方流動性 ${min_prev_low:.6f}'
                        })
    ...
    def _is_order_block_active(self, df: pd.DataFrame, ob_low: float, ob_high: float, ob_index: int) -> bool:
    ...
    def _is_fvg_filled(self, df: pd.DataFrame, gap_low: float, gap_high: float, gap_index: int) -> bool:
        ...
        for _, row in future_data.iterrows():
            if gap_low <= row['low'] <= gap_high or gap_low <= row['high'] <= gap_high:
                return True
        return False
